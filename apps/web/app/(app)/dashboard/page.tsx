  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Completed */}
        <div className="bg-background subtle-border crosshairs p-6 flex items-center gap-5">
          <div className="w-12 h-12 rounded border border-white/10 bg-white/[0.02] flex items-center justify-center text-white/80">
            <CheckCircle className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[10px] font-mono text-white/40 uppercase tracking-widest">Completed Sessions</p>
            <h3 className="text-2xl font-extrabold text-white mt-1">{stats?.completedSessions || 0}</h3>
          </div>
        </div>
        ...
        <div className="bg-background subtle-border crosshairs p-6 flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-mono text-white/40 uppercase tracking-widest">Average Rating</p>
        ...
        <div className="bg-background subtle-border crosshairs p-6 flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-mono text-white/40 uppercase tracking-widest">Skills Shared</p>
            <h3 className="text-2xl font-extrabold text-white mt-1">{stats?.skillsCount || 0}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-background subtle-border crosshairs p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              ...
              <Link href="/sessions" className="text-[10px] font-mono uppercase tracking-wider text-white/50 hover:text-white hover:underline transition-colors">
                View all
              </Link>
            </div>
            ...
            {upcomingSessions && upcomingSessions.length > 0 ? (
              <div className="space-y-3">
                {upcomingSessions.map((session: any) => {
                  ...
                  return (
                    <div key={session.id} className="p-4 rounded border border-white/5 bg-white/[0.01] hover:bg-white/[0.02] flex items-center justify-between gap-4 transition-all">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded border border-white/10 bg-white/[0.02] flex items-center justify-center text-xs font-mono font-bold text-white/80">
                          {session.skill.name.substring(0, 2).toUpperCase()}
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] font-mono uppercase tracking-wider text-white/60 bg-white/[0.02] border border-white/10 px-2.5 py-1 rounded flex items-center gap-1.5">
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              <h2 className="text-xs font-bold text-white font-mono uppercase tracking-wider flex items-center gap-2">
                ...
                {pendingRequests?.count > 0 && (
                  <span className="px-2 py-0.5 rounded bg-white text-black text-[9px] font-mono uppercase font-bold tracking-wider">
                    {pendingRequests.count} new
                  </span>
                )}
              </h2>
              <Link href="/requests" className="text-[10px] font-mono uppercase tracking-wider text-white/50 hover:text-white hover:underline">
                Open inbox
              </Link>
            </div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex gap-1.5">
                        <button
                          ...
                          className="px-3 py-1 bg-white text-black hover:bg-white/90 text-[9px] font-mono uppercase tracking-wider rounded transition-colors"
                        >
                          Accept
                        </button>
                        <button
                          onClick={() => handleRejectRequest(req.id)}
                          className="px-3 py-1 bg-white/5 hover:bg-white/10 text-[9px] font-mono uppercase tracking-wider text-white/70 border border-white/10 rounded transition-colors"
                        >
                          Decline
                        </button>
                      </div>
            ...
            {suggestedMatches && suggestedMatches.length > 0 ? (
              <div className="space-y-4 card-3d-wrapper">
                {suggestedMatches.map((m: any) => (
                  <div key={m.id} className="card-3d bg-background subtle-border crosshairs p-4 flex flex-col gap-3 transition-all group preserve-3d">
                    <div className="flex items-center gap-3 pop-depth-sm">
                      ...
                      <div className="min-w-0 flex-1">
                        <Link href={`/profile/${m.id}`} className="text-xs font-bold text-white hover:text-white hover:underline truncate block">
                          {m.name}
                        </Link>
                    ...
                    {m.matchExplanation && (
                      <div className="pop-depth-md p-2.5 rounded bg-white/[0.01] border border-white/5 text-[9px] text-white/60 leading-relaxed shadow-[0_0_10px_rgba(0,0,0,0.2)]">
                        <div className="flex items-center gap-1 font-semibold text-white/80 uppercase tracking-wider text-[8px] mb-0.5">
                          <Zap className="w-2.5 h-2.5 text-white/60 animate-pulse" />
                    ...
                    <Link href={`/profile/${m.id}`} className="pop-depth-md text-[9px] font-mono uppercase tracking-wider text-white/85 hover:text-white flex items-center gap-1 self-end group-hover:gap-1.5 transition-all">
