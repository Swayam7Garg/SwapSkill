    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-2">
            ...
            <span>Discover ExchangeSkill Peers</span>
          </h1>
          <p className="text-xs text-white/50 mt-1">
            Browse students, matching compatibility score is weighted higher for mutual teach/learn overlaps.
          </p>
        </div>
        <div className="text-xs text-white/40 font-semibold self-end md:self-auto bg-white/[0.02] border border-white/10 px-3 py-1.5 rounded">
          Showing <span className="text-white font-bold">{totalCount}</span> peers available
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-6 bg-rose-500/10 border border-rose-500/25 text-rose-400 rounded max-w-xl mx-auto text-center">
          ...
          <button onClick={fetchUsers} className="mt-3 text-xs bg-white text-black px-4 py-2 rounded font-bold uppercase font-mono tracking-wider">
            Retry Search
          </button>
        </div>
      ) : users.length === 0 ? (
        <div className="text-center py-20 glass-panel rounded border border-white/5 border-dashed max-w-lg mx-auto space-y-3">
          ...
          <button
            ...
            className="px-4 py-2 bg-white text-xs font-semibold text-black rounded uppercase font-mono tracking-wider"
          >
            Clear Filters
          </button>
        </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-4 border-t border-white/5 pt-6">
              <button
                ...
                disabled={page === 1}
                className="p-2 rounded border border-white/10 bg-white/5 text-white disabled:opacity-30 disabled:pointer-events-none hover:bg-white/10 transition-colors"
              >
